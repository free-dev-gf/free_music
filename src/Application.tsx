...
const Application: React.FC<Props> = (props) => {
    ...
    const currentList = list.find(a => a.id === currentListId) || { songs: [] };
    ...
    const playSong = (name: string) => {
        ...
        ipcRenderer.invoke('get-lyric', name.replace('.mp3', '.lrc')).then(res => {
            setLyric(res);
        });
    }
    ...
    const addSongToList = (listId: string) => {
        ipcRenderer.invoke('add-song-to-list', currentHandleSong, listId).then(data => {
            setList(data);
        });
    }
    ...
    const createTween = (e: any) => {
        ...
        const coords = { x: offsetLeft, y: offsetTop };
    ...
    const [currentArtist, currentName] = currentPlay.replace('.mp3', '').split('-').map(a => a.trim());
    ...
    return (
        <div className="free-music">
                ...
                <div className="free-music-content-song">
                    ...
                    <div className="free-music-content-song-list">
                        {renderSongs.map((completeName, idx) => {
                            const [ artist, name ] = completeName.replace('.mp3', '').split('-').map(a => a.trim());
                            return <SongItem
                                ...
                                onPlay={() => {
                                    if (playMode === EPlayMode.RANDOM) {
                                        setQueue(getRandomQueue(renderSongs.slice(), completeName));
                                    } else {
                                        setQueue(renderSongs.slice(renderSongs.indexOf(completeName)));
                                    }
                                    playSong(completeName);
                                }}
                                onContextMenu={e => onSongContextMenu({
                                    song: completeName,
                                    e,
                                })}
                                onAddToQueue={e => {
                                    ...
                                    const index = queue.indexOf(currentPlay);
                                    newQueue.splice(index + 1, 0, completeName);
                                    setQueue(newQueue);
                                }}
                            />
                        })}
            ...
            <Menu id={MENU_ID}>
                <Submenu label="添加到歌单">
                    {list.filter(a => !a.songs.includes(currentHandleSong)).map(a => (
                        <Item onClick={() => addSongToList(a.id)} key={a.id}>
                            {a.name}
                        </Item>    
                    ))}
                </Submenu>
                ...
                >
                    {Object.keys(config).map(key => (
                        <Form.Item
                            ...
                            label={key}
                            name={key}
                            rules={[{ required: true, message: 'Please input music direction!' }]}
                        >
                            <Input />
                        </Form.Item>
                    ))}
